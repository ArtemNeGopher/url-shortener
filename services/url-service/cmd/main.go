...
import (
	...
	"github.com/ArtemNeGopher/url-shortener/services/url-service/repository"

	pb "github.com/ArtemNeGopher/url-shortener/pkg/genproto/url"
	implGRPC "github.com/ArtemNeGopher/url-shortener/services/url-service/grpc"
...
func main() {
	...
	// Repository
	repo := repository.NewURLRepository(&config.Database, log)
	log.Debug("repository created")

	// GRPC
	implServer := implGRPC.NewServer(repo, config.URLCreateRetries, log)
	...
	// Register grpc
	pb.RegisterURLServiceServer(server, implServer)
